"""
Comprehensive tests for tritonparse.
Test Plan:
```
TORCHINDUCTOR_FX_GRAPH_CACHE=0 TRITONPARSE_DEBUG=1 python -m pytest tests/test_tritonparse.py -v
```
"""
...
@pytest.fixture
def triton_hooks_setup():
    """Fixture to manage triton hooks and compilation settings"""
    # Save original settings
    prev_listener = triton.knobs.compilation.listener
    prev_always_compile = triton.knobs.compilation.always_compile
    prev_jit_post_compile_hook = triton.knobs.runtime.jit_post_compile_hook
    prev_launch_enter_hook = triton.knobs.runtime.launch_enter_hook
    
    # Set up new settings
    triton.knobs.compilation.always_compile = True
    
    yield  # Provide control to test function
    
    # Always restore original settings, even if test fails
    triton.knobs.compilation.always_compile = prev_always_compile
    triton.knobs.compilation.listener = prev_listener
    triton.knobs.runtime.jit_post_compile_hook = prev_jit_post_compile_hook
    triton.knobs.runtime.launch_enter_hook = prev_launch_enter_hook


@pytest.mark.cuda
def test_extract_python_source_info(cuda_device, triton_hooks_setup):
    ...
    torch.manual_seed(0)
    size = (512, 512)
    a = torch.randn(size, device=cuda_device, dtype=torch.float32)
...
@pytest.mark.cuda
def test_whole_workflow(cuda_device, triton_hooks_setup):
    ...
    x = torch.randn(size, device=cuda_device, dtype=torch.float32)
    y = run_test_kernel(x)  # Run the simple kernel
    torch.cuda.synchronize()
    
    # Check that temp_dir_logs folder has content
    assert os.path.exists(
        temp_dir_logs), f"Log directory {temp_dir_logs} does not exist."
    log_files = os.listdir(temp_dir_logs)
    assert len(
        log_files) > 0, f"No log files found in {temp_dir_logs}. Expected log files to be generated during Triton compilation."
    print(f"Found {len(log_files)} log files in {temp_dir_logs}: {log_files}")
    
    tritonparse.utils.unified_parse(source=temp_dir_logs, out=temp_dir_parsed)
